...
<?php section('title', 'Create Event - SAMS'); ?>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="<?= route('events.store') ?>" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent <?= $errors->has('type') ? 'border-red-500' : '' ?>">
                        ...
                        <option value="church_service" <?= old('type') == 'church_service' ? 'selected' : '' ?>>Church Service</option>
                        <option value="special_program" <?= old('type') == 'special_program' ? 'selected' : '' ?>>Special Program</option>
                        <option value="week_of_emphasis" <?= old('type') == 'week_of_emphasis' ? 'selected' : '' ?>>Week of Emphasis</option>
                        <option value="idf" <?= old('type') == 'idf' ? 'selected' : '' ?>>Inter-Disciplinary Forum (IDF)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="semester_id" name="semester_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent <?= $errors->has('semester_id') ? 'border-red-500' : '' ?>">
                        <option value="">Select Semester (defaults to active)</option>
                        <?php foreach ($semesters as $semester): ?>
                        <option value="<?= e($semester->id) ?>" <?= (old('semester_id') == $semester->id || (!old('semester_id') && $activeSemester && $activeSemester->id == $semester->id)) ? 'selected' : '' ?>>
                            <?= e($semester->name) ?><?= $semester->is_active ? ' (Active)' : '' ?> - <?= e($semester->start_date->format('M d, Y')) ?> to <?= e($semester->end_date->format('M d, Y')) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                    <?php if ($activeSemester): ?><p class="mt-1 text-xs text-gray-500">If not selected, event will be assigned to: <?= e($activeSemester->name) ?></p><?php endif; ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" id="start_time" name="start_time" value="<?= e(old('start_time')) ?>" required
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="end_time" name="end_time" value="<?= e(old('end_time')) ?>" required
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent <?= $errors->has('description') ? 'border-red-500' : '' ?>"><?= e(old('description')) ?></textarea>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto border border-gray-300 rounded-lg p-4">
                        <?php $oldOfficers = old('officer_ids', []); ?>
                        <?php foreach ($officers as $officer): ?>
                        <label class="flex items-center">
                            <input type="checkbox" name="officer_ids[]" value="<?= e($officer->id) ?>" <?= in_array($officer->id, $oldOfficers) ? 'checked' : '' ?> class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="flex space-x-4 mt-8">
                <button type="submit" class="flex-1 bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Create Event</button>
